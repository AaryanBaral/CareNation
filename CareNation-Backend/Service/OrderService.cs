...
public class OrderService : IOrderService
{
    ...
    {
        // 1) Load order & ensure pending
        var order = await _orderRepo.GetOrderByIdAsync(orderId);
        if (order == null || order.Status != OrderStatus.Pending) return false;

        // 2) Reduce stock (throws if insufficient)
        await _productRepo.ReduceStockAsync(order.Items.Select(i => (i.ProductId, i.Quantity)));

        // 3) Mark approved in repo
        ...
        // Process commissions after approval
