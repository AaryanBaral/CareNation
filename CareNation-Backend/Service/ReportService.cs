...
public class ReportService : IReportService
{
    ...
    {
        return await _repo.GetTotalSalesByDistributorAsync(distributorId, from, to);
    }
        public async Task<List<SalesByTimeDto>> GetSalesByTimeForDistributorAsync(string distributorId, DateTime? from, DateTime? to, string period)
    {
        return await _repo.GetSalesByTimeForDistributorAsync(distributorId, from, to, period);
    }

    public async Task<IEnumerable<PaymentReadDto>> GetPaymentReportAsync(PaymentType? type, DateTime? from, DateTime? to)
        => await _repo.GetPaymentsAsync(type, from, to);

    public async Task<IEnumerable<PaymentReadDto>> GetPaymentReportForDistributorAsync(string userId, PaymentType? type, DateTime? from, DateTime? to)
        => await _repo.GetPaymentsByDistributorAsync(userId, type, from, to);
